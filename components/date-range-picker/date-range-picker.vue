	...
	export default {
		...
		methods: {
			...
			setDate() {
				...
				// 初始化完成后，同步备份数据
				this.backupResultDate = [...this.resultDate]
				this.backupPickerValue = [...this.pickerValue]
			},
			setPicker(date) {
			...
			pickerChange(e) {
				...
				// 月份处理，限制到当前月份
				if (this.years[currents[0]] === this.maxYear) {
				...
				// 限制到当前日期
				// if (this.years[currents[0]] === this.maxYear && this.months[currents[1]]*1 === currentMonth) {
			...
			close() {
				// 恢复日期显示文本
				this.resultDate = [...this.backupResultDate]
				// 恢复滚动条位置
				this.pickerValue = [...this.backupPickerValue]
				// 触发关闭事件
